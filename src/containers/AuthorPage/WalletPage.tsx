...
import { getNFTs, getNFTsForOwner } from 'utils/alchemy'
import CardNFTDisplay from "components/CardNFTDisplay";
import { useWeb3React } from "@web3-react/core";
import { Network } from "alchemy-sdk";
import { BeatLoader } from "react-spinners";
import CardNFTCuration from "components/CardNFTCuration";
...
const WalletPage: FC<AuthorPageProps> = ({ className = "" }) => {
 ...
 const [loading, setLoading] = useState(false)
  ...
  // const [nftData, setNFTData] = useState<any>()
    ...
    const {walletAddress: wallet} = useParams()
    const {
        email, 
        photoUrl,
        displayName,
...
const [ethNFTTotalCount, setEthTotalCount] = useState<number>()
const [polygonNFTTotalCount, setPolygonTotalCount] = useState<number>()


const _getNFTsForOwner = async (wallet: string) => {
    setLoading(true)
    const _nftData = await getNFTsForOwner(wallet, Network.ETH_MAINNET)
  
    const _nftDataPolygon = await getNFTsForOwner(wallet, Network.MATIC_MAINNET)
    // console.log('nftData', _nftData.ownedNfts)
    // @ts-ignore
    // setNFTData(_nftData)
    setNFTDataPOLYGON(_nftDataPolygon.ownedNfts)
    setPolygonTotalCount(_nftDataPolygon.totalCount)
    setNFTDataETH(_nftData.ownedNfts)
    setPolygonTotalCount(_nftData.totalCount)
    setLoading(false)
}
...
useEffect(() => {
    ...
    wallet && _getNFTsForOwner(wallet)
}, [wallet])
              ...
              <Tab.Panel className="">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10 mt-8 lg:mt-10">
                     ...
                     {nftDataETH && nftDataETH.map((nft: any, index: string) => {
                      ...
                      return (<CardNFTCuration key={index} nft={nft} unit='ETH'
                      chain={'ethereum'}
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10 mt-8 lg:mt-10">
                    ...
                    return (<CardNFTCuration key={index} nft={nft} unit='MATIC' 
                    chain={'ploygon'}
                    contractAddress={nft?.contract?.address}
                    tokenId={nft?.tokenId}
                    />)
